...
class SaveResult(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.better(compare_val, self.bestResult, sign):
            ...
            self.wait = 0
            # save ar here
            print 'save better results'
            numpy.savetxt('log/output/' + self.fileResult[:-4] + "_actual.csv", self.actual, delimiter=",")
            numpy.savetxt('log/output/' + self.fileResult[:-4] + "_estimate.csv", self.estimate, delimiter=",")
...
'''preditcting test outcome'''
def create_dense_multiclass(n_features, n_classes, hidden_node_size, nnet_model='dense', dropout=True):
    top_act = 'softmax'
...
'''predicting components:dense lstm feature'''
...
'''predicting component highway e2e'''
