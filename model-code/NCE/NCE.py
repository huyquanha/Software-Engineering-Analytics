...
class NCE(Layer): #NCE inherits from Layer
    def __init__(self, init='glorot_uniform', activation='linear',
                 input_dim=None, vocab_size=None, n_noise = 25, Pn=[0.5, 0.5],
                 weights=None,
                 W_regularizer=None, b_regularizer=None, activity_regularizer=None,
                 W_constraint=None, b_constraint=None,
                 bias=True, **kwargs): #kwargs allow passing keyworded variable length of arguments to function
        self.init = initializations.get(init) #use Glorot uniform distribution between +- sqrt(6 / (fan_in + fan_out))
        self.activation = activations.get(activation) #use linear activation function
        self.input_dim = input_dim
        self.vocab_size = vocab_size
        self.n_noise = n_noise
        self.Pn = theano.shared(numpy.array(Pn).astype(config.floatX)) #self.Pn is a shared variable now (used by different functions)
        self.rng = RS.RandomStreams(seed=SEED) #create a stream of random variables with various distributions

        self.W_regularizer = regularizers.get(W_regularizer) #apply penalties on weight during optimization
        self.b_regularizer = regularizers.get(b_regularizer) #apply penalties on bias during optimization
        self.activity_regularizer = regularizers.get(activity_regularizer) #apply penalties on activity during optimization
        ...
        if self.input_dim:
            kwargs['input_shape'] = (self.input_dim,) #add another argument named input_shape with a vlue of self.input_dim

        super(NCE, self).__init__(**kwargs)

    def build(self, input_shape): #define weights, regularlizers, constraints
        ...
        self.regularizers = [] #regularlizers is a list
        if self.W_regularizer: #if there is a weight regularlizer
            self.W_regularizer.set_param(self.W) #W_regularlizer will apply on self.W
            self.regularizers.append(self.W_regularizer)

        if self.bias and self.b_regularizer:
            self.b_regularizer.set_param(self.b) #similar for bias (provided there is a bias)
            self.regularizers.append(self.b_regularizer)

        if self.activity_regularizer:
            self.activity_regularizer.set_layer(self) #activity regularlizer on this layer
            self.regularizers.append(self.activity_regularizer)

        self.constraints = {} #constraints is a dictionary
        if self.W_constraint:
            self.constraints[self.W] = self.W_constraint #set the constraint for self.W as self.W_constraint
    ...
    def get_output_shape_for(self, input_shape): #specify the shape transformation logic (if layer modifies the shape of input)
        return (None, self.n_noise + 1)

    def call(self, inputs, mask=None): #where layer's logic lives
...
class NCE_seq(NCE): #NCE_seq inherits from NCE
    ...
    def call(self, inputs, mask=None):
        ...
        #generate n_noise samples from noise distribution Pn.
        noise_w = self.rng.choice(size=(n_samples, n_steps, self.n_noise), a=self.Pn.shape[0], p=self.Pn) #a is maxword. choose values from the random stream between [0,a]
        next_w = next_w.flatten().reshape([n_samples, n_steps, 1]) #flatten() collapse the array to 1D, and then reshape to n_samples, n_steps, 1
