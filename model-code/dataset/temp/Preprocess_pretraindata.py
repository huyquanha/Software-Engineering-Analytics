# this script reads <repo>_pretrain.csv to create dictionary for pretrain
# pretrain step is build on repo level
# input: <repo>_pretrain.csv
# output: <repo>._pretrain.pkl.gz, <repo>.dict.pkl.gz
...
def main():
    ...
    pretrain_path = 'data/' + sys.argv[1] + '.csv'
    pre_title, pre_descr = dataset.load_raw_text.load_pretrain(pretrain_path)
    ...
    f_pre = gzip.open('data/' + sys.argv[1] + '_pretrain.pkl.gz', 'wb')
    pkl.dump((pre_train, pre_valid, pre_valid), f_pre, -1)
    f_pre.close()

    f = gzip.open('data/' + sys.argv[1] + '.dict.pkl.gz', 'wb')
