...
def main():
    # load training data:
    data_path = 'data/' + sys.argv[1] + '.csv'
    print data_path
    title, description, labels = dataset.load_raw_text.load(data_path)

    print len(title)
    print len(description)
    print len(labels)
    print '----'
    f = open('data/' + sys.argv[1] + '_3sets.txt', 'r')
    ...
    f_dict = gzip.open('data/' + sys.argv[2] + '.dict.pkl.gz', 'rb')
    ...
    f = gzip.open('data/' + sys.argv[1] + '.pkl.gz', 'wb')
